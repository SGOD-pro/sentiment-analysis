...
export default function Reports() {
  ...
  return (
    <div className="flex pt-14 min-h-screen">
      {/* Sidebar — minimal on Reports per Stitch design */}
      <aside className="fixed left-0 top-14 h-[calc(100vh-56px)] w-60 border-r border-border bg-card p-5 z-40 hidden md:flex flex-col gap-4">
      ...
      <main className="md:pl-60 flex-1 p-6 space-y-5">
        {/* Page header */}
            ...
            {loading ? <Skeleton className="h-64" /> : (
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={distData} margin={{ top: 20 }}>
                  ...
                  <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} tickFormatter={(v) => `${v}%`} domain={[0, 100]} />
                  <Tooltip formatter={(v) => [`${v}%`, "Share"]} contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }} />
                  <Bar dataKey="pct" radius={[4, 4, 0, 0]} label={{ position: "top", fill: "hsl(var(--foreground))", fontSize: 12, fontWeight: 600 }}>
