...
function Sidebar({
  ...
  return (
    <aside className="fixed left-0 top-14 h-[calc(100vh-56px)] w-60 border-r border-border bg-card flex flex-col p-5 gap-5 z-40 hidden md:flex">
...
function StatCard({ label, value, sub, icon: Icon, delta, deltaUp }: {
  ...
  return (
    <Card className="hover:shadow-lg transition-shadow duration-200">
      <CardContent className="pt-5 pb-5 flex flex-col justify-between h-36 relative overflow-hidden">
        ...
        <div>
          <div className="text-4xl font-bold font-number leading-tight">{value}</div>
          {sub && <div className="text-[10px] text-muted-foreground uppercase tracking-wider mt-1">{sub}</div>}
        </div>
      </CardContent>
    </Card>
  );
...
export default function Dashboard() {
  ...
  return (
    <div className="flex pt-14 min-h-screen">
      <Sidebar catCol={colMap.catCol} categories={categories} onFilter={setFilters} />

      <main className="md:pl-60 flex-1 p-6 space-y-6 max-w-[1440px]">
        {/* Alert strip */}
              ...
              {loading ? <Skeleton className="h-60 w-full" /> : issues.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={issues.slice(0, 8)} layout="vertical" margin={{ left: 0, right: 8 }}>
                    ...
                    <YAxis type="category" dataKey="issue_tag" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} width={110}
